...
exports.all = function all(params, fn) {
  ...
  Topic
  .find(query)
  .select('id topicId mediaTitle tag participants votes createdAt updatedAt closingAt publishedAt deletedAt status open closed links user coverUrl author authorUrl forum keywords')
  .populate('tag', 'id hash name color image')
  .populate('forum', 'id name title summary imageUrl')
  .populate('user', 'avatar profilePictureUrl displayName firstName lastName party')
