...
exports.create = function create (data, fn) {
  ...
  Topic.get(comment.topicId.toString(), function (err, topic) {
    if (err) return _handleError(err, fn);
    if (!topic) return _handleError(new Error('Topic not found.'), fn);
    if (topic.closed) return _handleError(new Error('Topic is closed.'), fn);
    ...
    comment.save(function (err) {
      ...
      User.populate(comment, { path: 'author' }, function (err) {
        ...
        if (topic.forum) {
          topicUrl = utils.buildUrl(config, { pathname: config.subPath + '/' + topic.forum.name + '/consulta/' + comment.reference });
        } else {
          topicUrl = utils.buildUrl(config, { pathname: config.subPath + '/consulta/' + comment.reference });
        }
...
exports.remove = function remove (comment, fn) {
  comment.remove(function (err) {
    ...
    fn(null);
    updateTopicParticipantCountFromComment(comment);
  });
};

function updateTopicParticipantCountFromComment (comment) {
  if (!comment.topicId) return;

  Topic.get(comment.topicId.toString(), function (err, topic) {
    if (err) return log('Found error: %j', err);
    Comment.find({
      topicId: comment.topicId,
      author: comment.author
    }).limit(1).exec(function (err, comments) {
      if (err) return log('Found error: %j', err);

      if (comments.length) {
        log('Participants updated on topic: %j', topic._id);
        topic.participants.addToSet(comment.author);
      } else {
        log('Participant remove from topic: %j', topic._id);
        topic.participants.remove(comment.author);
      }

      if (topic.isModified()) topic.save();
    });
  });
}
