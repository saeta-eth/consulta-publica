...
let valid = (ctx, next) => {
  var page = ctx.params.page || "perfil";
  var valid = ['perfil', 'contraseña', 'notificaciones', 'forums'];
...
page('/ajustes/:page?', valid, external, user.required, (ctx, next) => {
  if (!ctx.valid) {
    return next();
  }

  let page = ctx.params.page || "perfil";
  ...
  o('[href="' + config.subPath + '/ajustes/' + page + '"]', container).parent('li').addClass('active');

  // Set page's title
  title(o('[href="#{config.subPath}/ajustes/' + page + '"]').html());
  ...
  console.log('togle ajustes views ', page)
  // Display current settings page
  page = (page === 'contraseña')?'contrasena':page;
