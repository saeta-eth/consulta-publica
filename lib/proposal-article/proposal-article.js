...
export default class ProposalArticle extends view('appendable', 'withEvents') {
  ...
  switchOn () {
    ...
    this.bind('click', '.side-comment', this.onSideCommentActive.bind(this));
  }
  ...
  onSideCommentActive(){
    debugger
    var commentables = this.el.querySelectorAll('.commentable-section');
    var active = this.el.querySelector('.side-comment.active').parentNode;
    
    for(var i = 0; commentables.length > i; i++)
      commentables[i].className = commentables[i].className + ' fadeOut';

    active.className = active.className.replace(' fadeOut', '');

    document.getElementById('#content').addEventListener('click', function clickOut(e){
      for(var i = 0; commentables.length > i; i++)
        commentables[i].className = commentables[i].className.replace(' fadeOut', '');
