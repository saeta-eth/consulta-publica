...
var topicListKeys = [
  ...
  'tag participants voted createdAt updatedAt closingAt closingMessage',
  'publishedAt deletedAt votable clauseTruncationText links author authorUrl user coverUrl keywords'
].join(' ');
...
app.post('/topic/create', restrict, hasAccess, function (req, res, next) {
  ...
  api.topic.create(req.body, function (err, topic) {
    ...
    var keys = [
      ...
      'publishedAt deletedAt votable bodyTruncationText links author authorUrl user coverUrl keywords'
    ].join(' ');
    res.json(exposeTopic(topic, req.user));
  });
});
...
app.post('/topic/:id/publish', restrict, hasAccess, function (req, res) {
  ...
  api.topic.get(req.params.id, function (err, topic) {
    ...
    topic.save(function (err, saved) {
      ...
      var topicUrl = utils.buildUrl(config, { pathname: config.subPath + prefix + '/consulta/' + topic.id });
