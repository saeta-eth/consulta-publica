...
var topicListKeys = [
  ...
  'publishedAt deletedAt votable clauseTruncationText links user coverUrl keywords'
].join(' ');
...
app.get('/topic/all',
  ...
  function (req, res) {
    ...
    })//.populate('user', 'displayName profilePictureUrl');
  }
);
...
app.post('/topic/create', restrict, hasAccess, function (req, res, next) {
  ...
  api.topic.create(req.body, function (err, topic) {
    ...
    var keys = [
      ...
      'publishedAt deletedAt votable bodyTruncationText links user coverUrl keywords'
    ].join(' ');
    res.json(exposeTopic(topic, req.user));
  });
});
