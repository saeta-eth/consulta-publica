...
var defaultCategory = require('../../comments/categories/values').names[0];
...
app.get('/comment/all', requireTopicId, function (req, res) {
  ...
  var author = req.query.author ? req.query.author : req.query.excludeAuthor ? {'$ne': req.query.excludeAuthor} : null;
  var category = req.query.category || defaultCategory;
...
app.post('/comment', restrict, requireTopicId, function (req, res) {
  ...
  api.comment.create(comment, function (err, commentDoc) {
    ...
    var keys = [
      ...
      'createdAt replies context reference category'
    ].join(' ');
