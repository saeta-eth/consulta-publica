...
export default class CommentsView extends FormView {
  ...
  constructor (item, reference) {
    ...
    this.category = this.categories.default.name;
  ...
  onsuccess (res) {
    this.commentFormInit();
    this.categories.select(res.body.category);
    this.add(res.body);
    this.track(res.body);
  }
  ...
  selectCategoryInput (e) {
    var category = e.delegateTarget.getAttribute('data-category');
    var categoryInput = this.find('.categories-input input')[0];
    ...
    if(this.find('.form-group:not(.categories-input)')[0].style.visibility === 'hidden')
  ...
  activateCategoryInput (name) {
    var siblings = this.find('.categories-input span[data-category]');
    var active = this.find('.categories-input span[data-category=' + name + ']')[0];

    for(var i = 0; i < siblings.length; i++)
      siblings[i].querySelector('.category-label').style.color = '';
    ...
    active.querySelector('.category-label').style.color = active.getAttribute('data-color');
  };
  ...
  commentFormInit () {
    ...
    for(var i = 0; i < formChilds.length; i++)
      formChilds[i].style.visibility = 'hidden';
    this.textarea.value = '';
    this.find('.new-comment')[0].style.display = 'block';
  ...
  commentFormCategories () {
    this.find('.categories-input')[0].style.visibility = 'visible';
    this.find('.new-comment')[0].style.display = 'none';
  ...
  commentFormText () {
    var formChilds = this.form[0].children;
    for(var i = 0; i < formChilds.length; i++)
      formChilds[i].style.visibility = 'visible';
  };
}
