...
export default class CommentsView extends FormView {
  ...
  toggleMyComments (e) {
    log('toggle my comments', e.delegateTarget.querySelector('input').checked);
    window.hash = e.delegateTarget.querySelector('input').checked ? '#user' : '#all';
  };
  ...
  activateCategoryInput (name) {
    var siblings = document.querySelectorAll('.categories-input span[data-category]');
    var active = document.querySelector('.categories-input span[data-category=' + name + ']');
    ...
    for(var i = 0; i < siblings.length; i++)
      siblings[i].querySelector('.category-label').className = '#818181';//!~siblings[i].querySelector('.category-label').className.indexOf('inactive') ? 'inactive';
  ...
  selectCategoryInput (e) {
    ...
    document.querySelector('.categories-input input').value = category;
