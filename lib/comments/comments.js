...
export default class CommentsView extends FormView {
  ...
  switchOn () {
    ...
    this.bind('click', '.cancel-new-comment', 'cancelNewComment')
  ...
  /**
   * Cancel new comment hide form / reser
   *
   * 
   * @api public
   */
  ...
  initializeCommentsForm(){
    this.activateCategoryInput(this.category);
    this.commentFormDisplay().hide();
  }  
  ...
  selectCategoryInput (e) {
    ...
    if(!this.commentFormDisplay().status)
      this.commentFormDisplay().show();
  };
  ...
  commentFormDisplay () {
    var status = false;
    var hiddeables = this.find('.form-group:not(.categories-input)');

    function toggle(bool){
      return function(){
        for(var i = 0; i < hiddeables.length; i++)
          hiddeables[i].style.display = (bool) ? 'block' : 'none';
        status = false;
      }
    }
