...
export default class CommentsView extends FormView {
  ...
  constructor (item, reference) {
    ...
    this.comments = [];
    this.commentsContainer = this.find('.comments-list');
  ...
  add (comment) {
    ...
    if (Array.isArray(comment)) {
      if (comment.length > 50) {
        this.add(comment.slice(0, 49))
        window.requestAnimationFrame(() => {
          this.add(comment.slice(50, comment.length - 1))
        })
      } else {
    ...
    comment.closed = this.item.closed
    var card = new CommentCard(comment);
    this.comments.push(comment);

    card.appendTo(this.commentsContainer);
  ...
  commentFormInit () {
    ...
    if(user.id)
      this.textarea.value = '';
    this.find('.new-comment')[0].className = this.find('.new-comment')[0].className.replace(' hide', '');
