...
page(forumRouter('/admin/*'),
  ...
  (ctx, next) => {
    ...
    ctx.content = container.querySelector('.admin-content');
...
page('/admin', ctx => {
  page.redirect('/admin/' + (config.multiForum ? 'forums' : 'topics'));
});

if (config.multiForum) {
  require('../admin-forums/admin-forums');
}
...
require('../admin-users/admin-users');
require('../admin-comments/admin-comments');
...
function valid (ctx, next) {
  ...
  if (/forums|topics|tags|users|whitelists|comments/.test(section)) return next();
