...
page('/ayuda/:page?', valid, parse, (ctx, next) => {
  ...
  let menuItem = o('a[href*="/ayuda/' + current + '"]', container);
...
function valid(ctx, next) {
  ...
  var valid = ['markdown'];
  if (config.frequentlyAskedQuestions) valid.push('como-funciona');
  if (config.termsOfService) valid.push('terminos-y-condiciones');
  if (config.privacyPolicy) valid.push('privacidad');
  if (config.glossary) valid.push('glossary');
  if (config.aboutUs) valid.push('acerca');
