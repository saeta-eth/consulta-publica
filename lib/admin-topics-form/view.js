...
export default class TopicForm extends FormView {
  ...
  constructor(topic, forum, forums, tags) {
    ...
    tagsInput(this.find('input[type=tags]')[0]);

    var keywords = this.locals.topic.keywords;
    if(keywords){
      var keywordsDiv = this.find('.tags-input')[0];
      keywords.forEach(function(k){
        var kSpan = document.createElement('span');
        kSpan.setAttribute('data-tag', k);
        kSpan.textContent = k;
        kSpan.className = 'tag';
        keywordsDiv.insertBefore(kSpan, keywordsDiv.querySelector('.tags-input>input'));
      });
    }
  }
  ...
  postserialize(data) {
    ...
    data.keywords = [];
    this.find('.tags-input span[data-tag]')
      .map(function(v){ return v.getAttribute('data-tag'); })
      .forEach(function(v){ data.keywords.push(v);  });
